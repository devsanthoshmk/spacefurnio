<template>
  ...
  >
    ...
    >
      ...
      >
        <div v-for="(img, idx) in product.images" :key="idx" class="w-full h-full flex-shrink-0">
          <img
            :src="img"
            :alt="product.name"
      ...
      >
        <div class="flex gap-1.5 bg-black/20 backdrop-blur-sm px-2 py-1 rounded-full">
          <button
            ...
            :class="
              currentImageIndex === idx ? 'w-4 bg-white' : 'w-1.5 bg-white/60 hover:bg-white/80'
            "
          ></button>
        </div>
      </div>
      ...
      <div class="absolute top-3 left-3 flex flex-col gap-1 pointer-events-none">
        <span
          v-if="isNew"
          class="px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-white bg-brand shadow-lg rounded-full"
          >New</span
        >
        <span
          v-if="product.discount"
          class="px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-white bg-rose-500 shadow-lg rounded-full"
    ...
    <div class="mt-4 flex flex-col flex-1 px-1">
      <div class="flex justify-between items-start mb-1">
        ...
        <div
          v-if="product.rating"
          class="flex items-center gap-1 bg-gray-50 px-1.5 py-0.5 rounded-md"
        >
      ...
      <h3
        class="text-base font-semibold text-gray-900 line-clamp-1 group-hover:text-brand transition-colors"
      >
        <a :href="productUrl">
      ...
      <div class="mt-auto pt-3 flex items-end justify-between">
        ...
        <div
          ...
          class="flex -space-x-1.5 overflow-hidden pl-2 pb-1"
        >
          <div
          ...
          <div
            ...
            class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-gray-100 ring-2 ring-white text-[9px] font-bold text-gray-500"
          >
