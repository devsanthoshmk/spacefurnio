...
function transformProduct(row) {
    ...
    return {
        ...
        description: row.description || `Elevate your space with this beautifully crafted ${(row.name || '').toLowerCase()}. Made with premium ${(row.material_name || 'materials').toLowerCase()}, this piece combines timeless design with exceptional quality.`,
        features: [
...
export async function getCategories() {
    ...
    try {
      ...
      GROUP BY category_id`
        ...
        const countMap = {}
        counts.forEach(r => { countMap[r.category_id] = r.cnt })
...
export async function getStyles() {
    ...
    try {
        ...
        const data = rows.map(st => ({
            ...
            name: st.name,
            slug: st.slug,
            image: STYLE_IMAGE_MAP[st.slug] || 'https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?w=600&h=600&fit=crop',
...
export async function getFilterOptions(category = null) {
    try {
        const catFilter = category
            ? `JOIN categories c ON c.id = p.category_id WHERE c.slug = $1`
            : ''
        const params = category ? [category] : []

        const [brandsRes, materialsRes, colorsRes, priceRes] = await Promise.all([
            sql.query(`SELECT DISTINCT b.name FROM products p JOIN brands b ON b.id = p.brand_id ${catFilter} ORDER BY b.name`, params),
            sql.query(`SELECT DISTINCT m.name FROM products p JOIN materials m ON m.id = p.material_id ${catFilter} ORDER BY m.name`, params),
            sql.query(`SELECT DISTINCT cl.name, cl.hex_code FROM product_colors pc JOIN colors cl ON cl.id = pc.color_id JOIN products p ON p.id = pc.product_id ${catFilter} ORDER BY cl.name`, params),
            sql.query(`SELECT COALESCE(MIN(p.price_cents)/100, 0) AS min, COALESCE(MAX(p.price_cents)/100, 5000) AS max FROM products p ${catFilter}`, params),
        ])
